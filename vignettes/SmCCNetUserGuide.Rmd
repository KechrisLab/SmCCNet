...
First, we need to split the data ($X_1, X_2, Y$) into test and training sets (Figure \@ref(fig:CVflow), Step I.1). The data sets will be standardized (centered and scaled) by columns within the algorithm. Since data with zero column variance can not be scaled, we require each column variance to be greater than zero for the test and training sets. To ensure the validability of the data sets, we recommend to check if they are scalable first and save the standardized data under a CV directory.
